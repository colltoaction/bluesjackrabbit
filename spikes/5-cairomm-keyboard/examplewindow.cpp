...
ExampleWindow::ExampleWindow()
{
	...
	// Events. Quiero key pressed y key released
	add_events(Gdk::KEY_PRESS_MASK | Gdk::KEY_RELEASE_MASK);

	this->signal_key_press_event().connect(sigc::mem_fun(*this, &ExampleWindow::on_key_press_event) );
	m_entry.signal_key_press_event().connect(sigc::mem_fun(*this, &ExampleWindow::on_key_press_event) );

	m_entry.signal_key_release_event().connect(sigc::mem_fun(*this, &ExampleWindow::entryKeyRelease));
...
bool ExampleWindow::on_key_press_event(GdkEventKey* event) {
	...
	switch (event->keyval) {
        case GDK_KEY_Left:
        	this->surface->moverIzq();
        	break;
        case GDK_KEY_Right:
            this->surface->moverDer();
            break;
        case GDK_KEY_Up:
            this->surface->moverArriba();
            break;
        case GDK_KEY_Down:
